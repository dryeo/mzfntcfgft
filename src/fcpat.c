...
typedef struct _FcObjectStaticNameList
{
  void *pPtr;
  void *pNext;
} FcObjectStaticNameList_t, *FcObjectStaticNameList_p;

static FcObjectStaticNameList_p pFcObjectStaticNameListHead = NULL;

void
FcObjectStaticNameListInit()
{
  pFcObjectStaticNameListHead = NULL;
}

void
FcObjectStaticNameListFini()
{
  FcObjectStaticNameList_p pToDelete;

  while (pFcObjectStaticNameListHead)
  {
    pToDelete = pFcObjectStaticNameListHead;
    pFcObjectStaticNameListHead = pFcObjectStaticNameListHead->pNext;
...
void
FcObjectStaticNameListAdd(void *pPtr)
{
  FcObjectStaticNameList_p pNew;

  pNew = (FcObjectStaticNameList_p) malloc(sizeof(FcObjectStaticNameList_t));
  if (pNew)
  {
    pNew->pPtr = pPtr;
    pNew->pNext = pFcObjectStaticNameListHead;
    pFcObjectStaticNameListHead = pNew;
  }
}
