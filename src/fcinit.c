...
static FcChar8 achOS2FontDir[512];

static FcChar8 *
FcGetOS2DefaultFontDir (void)
{
    ULONG ulSysInfo;
    APIRET rc;

    rc=DosQuerySysInfo(QSV_BOOT_DRIVE, QSV_BOOT_DRIVE, &ulSysInfo, sizeof(ulSysInfo));
    if (rc!=NO_ERROR)
        sprintf(achOS2FontDir, "c:/PSFONTS");
    else
        sprintf(achOS2FontDir, "%c:/PSFONTS", (unsigned char) (ulSysInfo-1+'A'));

    return &(achOS2FontDir[0]);
}
#endif
...
FcInitFallbackConfig (void)
{
    ...
    /* It may not exist yet, so try to create it, and try to open it again! */
    if (!FcCreateDefaultConfigFile())
        goto bail1;
    /* Add default font dir of OS/2 */
    if (!FcConfigAddDir (config, FcGetOS2DefaultFontDir()))
        goto bail1;
#else
